...
function evaluarPassword(val) {
    ...
    if (fill) { fill.style.width = nivel.width; fill.style.background = nivel.color; }
    if (text) { text.textContent = nivel.label; text.style.color = nivel.color; }
}
...
================================================================ */
document.querySelectorAll('.form-validable input, .form-validable select, .form-validable textarea')
...
document.addEventListener('DOMContentLoaded', function () {
    if (window.Preferencias && Preferencias.init) {
        Preferencias.init();
    }
    ...
    if (btnGuardar) {
        btnGuardar.addEventListener('click', function () {
            ...
            if (!sel) {
                var msg = (window.Preferencias && Preferencias.t)
                    ? Preferencias.t('notif.selecciona')
                    : 'Selecciona una preferencia.';
                if (window.Toast) Toast.mostrar(msg, 'warning');
                return;
            }
            btnGuardar.disabled = true;
            var html = btnGuardar.innerHTML;
            var guardando = (window.Preferencias && Preferencias.t)
        ...
        fetch(BASE_URL + '/app/controllers/preferenciasNotificacionController.php?accion=historial&limite=10')
            ...
            .catch(function () {
                ...
                if (sinH) {
                    ...
                    sinH.textContent = (window.Preferencias && Preferencias.t)
            ...
            fetch(BASE_URL + '/cliente/cerrar-sesion', {
                ...
                .then(function (data) {
                    if (data.status === 'success') {
                        ...
                        if (window.Toast) Toast.mostrar('Sesión cerrada.', 'success');
                    } else {
                        btn.disabled = false;
                        btn.innerHTML = '<i class="bi bi-box-arrow-right"></i> Cerrar';
                        if (window.Toast) Toast.mostrar('No se pudo cerrar la sesión.', 'error');
                    }
                })
                .catch(function () {
                    btn.disabled = false;
                    btn.innerHTML = '<i class="bi bi-box-arrow-right"></i> Cerrar';
                    if (window.Toast) Toast.mostrar('Error de conexión.', 'error');
                });
        });
    });
    ...
    if (btnTodasSesiones) {
        btnTodasSesiones.addEventListener('click', function () {
            ...
            btnTodasSesiones.innerHTML = '<i class="bi bi-hourglass-split"></i> Cerrando…';
            fetch(BASE_URL + '/cliente/cerrar-todas-sesiones', { method: 'POST' })
                ...
                .then(function (data) {
                    if (data.status === 'success') {
                        ...
                        if (window.Toast) Toast.mostrar('Todas las sesiones cerradas.', 'success');
                    } else {
                        btnTodasSesiones.disabled = false;
                        btnTodasSesiones.innerHTML = '<i class="bi bi-box-arrow-right"></i> Cerrar todas las otras sesiones';
                        if (window.Toast) Toast.mostrar('No se pudieron cerrar las sesiones.', 'error');
                    }
                })
                .catch(function () {
                    btnTodasSesiones.disabled = false;
