...
const Preferencias = (() => {
    ...
    async function _cargarStrings(idioma) {
        try {
            const r = await fetch(`${BASE_URL}/cliente/api/preferencias/lang?idioma=${idioma}`);
    ...
    function _aplicarTema(tema) {
        ...
        /* Sincronizar botones de tema si existen */
        document.querySelectorAll('[data-accion-tema]').forEach(btn => {
            btn.classList.toggle(
                'tema-btn--activo',
                btn.dataset.accionTema === tema
            );
        });
    }
    ...
    async function guardar(cambios) {
        try {
            ...
            if (data.status === 'ok') {
                ...
                /* Re-aplicar si cambió el idioma o el tema */
                if (cambios.tema)    _aplicarTema(cambios.tema);
                if (cambios.idioma) {
    ...
    function _inicializarEventos() {
        ...
        document.querySelectorAll('[data-accion-tema]').forEach(btn => {
            btn.addEventListener('click', () => {
                const tema = btn.dataset.accionTema;
                _aplicarTema(tema);   // aplicar inmediato (sin esperar respuesta)
                guardar({ tema });
            });
        });

        /* Radio de notificaciones */
        document.querySelectorAll('input[name="preferencia_notificacion"]').forEach(radio => {
    ...
    function _inicializarAvatar() {
        document.querySelectorAll('img[data-avatar-fallback]').forEach(img => {
            ...
            img.addEventListener('error', () => _reemplazarConIniciales(img), { once: true });
        });
    }
    ...
    function _reemplazarConIniciales(img) {
        const iniciales = (img.dataset.avatarFallback || '?').toUpperCase().slice(0, 2);
...
function cerrarSesion(token, btnEl) {
    ...
    .then(data => {
        if (data.status === 'ok') {
            const item = document.querySelector(`.sesion-item[data-token="${token}"]`);
            if (item) {
                ...
                setTimeout(() => {
                    item.remove();
                    const restantes = document.querySelectorAll('.sesion-item:not(.sesion-item--actual)');
                    if (!restantes.length) {
            ...
            Toast.mostrar(mensajes[data.code] || Preferencias.t('seg.sesion.error'), 'error');
        }
    })
    .catch(() => {
        btnEl.disabled = false;
...
function cerrarTodasSesiones(btnEl) {
    ...
    .then(data => {
        if (data.status === 'ok') {
            document.querySelectorAll('.sesion-item:not(.sesion-item--actual)').forEach(el => el.remove());
            document.querySelector('.sesiones-footer')?.remove();
...
function previewFoto(event) {
    ...
    reader.onload = e => {
        ...
        const avatar = document.querySelector('.avatar-grande img, .avatar-grande canvas');
        if (!avatar) return;
...
function checkPasswordStrength() {
    ...
    if (!password.length) { indicator.style.display = 'none'; return; }
...
function checkPasswordMatch() {
    ...
    if (!confirmar.length) { indicator.innerHTML = ''; return; }
...
function cambiarTab(tab) {
    document.querySelectorAll('.tab-content').forEach(el => el.classList.add('tab-content--hidden'));
...
function toggleFAQ(btn) {
    ...
    document.querySelectorAll('.faq-item.active').forEach(el => {
        ...
        el.querySelector('.faq-question i')?.style.setProperty('transform', 'rotate(0deg)');
    });
