class ListaPropietarios {
    ...
    cacheDom() {
        ...
        this.inputBuscarPropietarios =
            document.getElementById("buscarPropietarios");
    ...
    exportarCSV() {
        try {
            ...
            let csv =
                "foto Documento, Nombres y Apellidos, Teléfono, Email, Estado\n";
        ...
        fetch(
            ...
            .then((data) => {
                ...
                if (data.propietarios) {
                    this.crearTablaPropietarios(data.propietarios);
                }
            })
            .catch((error) => {
    ...
    crearTablaPropietarios(propietarios) {
        this.tablaPropietarios.clear();
        propietarios.forEach((propietario) => {
            this.tablaPropietarios.row
                .add([
                    propietario.numero_documento,
                    `${propietario.nombre}`,
                    propietario.telefono,
                    propietario.email,
                    propietario.estado.toLowerCase() === "activo" ?
                    '<span class="estado-activo">Activo</span>' :
                    '<span class="estado-inactivo">Inactivo</span>',
                    propietario.estado.toLowerCase() === "activo" ?
                    `<button class="btn-accion btn-eliminar" title="Eliminar" data-id="${propietario.id_propietario}">
