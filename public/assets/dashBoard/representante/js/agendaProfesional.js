...
class AgendaProfesional {
  ...
  cacheDom() {
    ...
    this.segundaFranjaHorarioContainer = document.getElementById(
      "segundaFranjaHorarioContainer",
    );
  ...
  cargarDatosEdicion($data) {
    ...
    document.getElementById("edit_id_disponibilidad").value =
      disponibilidad.id_disponibilidad;
    document.getElementById("edit_dia").value = disponibilidad.dia_semana;
    document.getElementById("edit_hora_inicio").value =
    ...
    document.getElementById("edit_especialidad").value =
      disponibilidad.id_especialidad;
  }
  ...
  crearAgendaProfesional(data) {
    ...
    const tieneSegundoHorario =
      data.hora_inicio_seccond || data.hora_fin_seccond;
    const segundoHorarioCompleto =
      data.hora_inicio_seccond && data.hora_fin_seccond;
    ...
    if (!primerHorarioCompleto && !segundoHorarioCompleto) {
      this.mostrarMensajeError(
        "Horario incompleto",
        "Debe ingresar al menos un par completo de horarios (inicio y fin)",
      );
      return;
    }
    ...
    if (tienePrimerHorario && !primerHorarioCompleto) {
      this.mostrarMensajeError(
        "Horario de la mañana incompleto",
        "Si ingresa un horario en la mañana, debe completar tanto la hora de inicio como la hora de fin",
      );
      return;
    }
    ...
    if (tieneSegundoHorario && !segundoHorarioCompleto) {
      this.mostrarMensajeError(
        "Horario de la tarde incompleto",
        "Si ingresa un horario en el horario de la tarde, debe completar tanto la hora de inicio como la hora de fin",
      );
      return;
    }
    ...
    if (primerHorarioCompleto && segundoHorarioCompleto) {
      ...
      const horaInicioTarde = new Date(
        `1970-01-01T${data.hora_inicio_seccond}:00`,
      );
      const horaFinTarde = new Date(`1970-01-01T${data.hora_fin_seccond}:00`);

      if (horaInicioTarde < horaFinManana) {
        this.mostrarMensajeError(
          "Horario de la tarde inválido",
          "El horario de la tarde no puede ser menor al horario de la mañana",
        );
        return;
      }
    ...
    // Se usa el action del form para que funcione tanto en representante como en veterinario
    const endpoint =
      this.formAgendaRegister?.action ||
      `${baseUrl}/representante/agregar-disponibilidad-agenda`;
    fetch(endpoint, {
      method: "POST",
      headers: {
