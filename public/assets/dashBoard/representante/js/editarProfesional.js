...
class EditarProfesional {
  ...
  cacheDom() {
    ...
    this.inputEspecialidadesCargadas = document.getElementById(
      "especialidadesCargadasInput",
    );
    this.especialidades = this.$$(".check-especialidades");
    this.containerEspecialidades = document.getElementById(
    ...
    this.inputServiciosCargados = document.getElementById(
      "serviciosCargadosInput",
    );
  ...
  bindEvents() {
    ...
    if (this.listServicios) {
      document.addEventListener("click", (event) => {
        const isClickInside =
          this.listServicios.contains(event.target) ||
          this.btnAgregarServ.contains(event.target);
        if (!isClickInside) {
    ...
    if (this.listEspecialidades) {
      document.addEventListener("click", (event) => {
        const isClickInside =
          this.listEspecialidades.contains(event.target) ||
          this.btnAgregarEsp.contains(event.target);
        if (!isClickInside) {
    ...
    if (this.inputServiciosCargados) {
      if (this.inputServiciosCargados.value.length > 0) {
        ...
        const listaCombinada = [
          ...listServCargadas?.map((s) => ({
            id: s.id_servicio,
            name: s.nombre,
          })),
          ...(valorServSeleccionadas.length > 0
            ? JSON.parse(valorServSeleccionadas)
            : []),
        ];
  ...
  toggleEspecialidades() {
    ...
    if (
      this.inputEspecialidades.value.length > 0 ||
      this.inputEspecialidadesCargadas.value.length > 0
    ) {
      const listEsp =
        this.inputEspecialidades.value.length > 0
          ? JSON.parse(this.inputEspecialidades.value)
          : [];
      const listEspCargadas =
        this.inputEspecialidadesCargadas.value.length > 0
          ? JSON.parse(this.inputEspecialidadesCargadas.value)
          : [];
      ...
      if (listaCombinada && listaCombinada.length > 0) {
        this.especialidades.forEach((e) => {
          const espId = e.value;
          const encontrado = listaCombinada.some(
            (esp) => parseInt(esp.id_especialidad) === parseInt(espId),
          );
          if (encontrado) {
            e.checked = true;
          } else {
    ...
    if (this.inputEspecialidadesCargadas.value.length > 0) {
      const listEspCargadas = JSON.parse(
        this.inputEspecialidadesCargadas.value,
      );
      if (listEspCargadas && listEspCargadas.length > 0) {
        this.especialidades.forEach((e) => {
          const espId = e.value;
          const encontrado = listEspCargadas.find(
            (esp) => esp.id_especialidad === parseInt(espId),
          );
          if (encontrado) {
            e.disabled = true;
          }
  ...
  toggleServicios() {
    ...
    if (
      this.inputServicios.value.length > 0 ||
      this.inputServiciosCargados.value?.length > 0
    ) {
      const listServ =
        this.inputServicios.value.length > 0
          ? JSON.parse(this.inputServicios.value)
          : [];
      const listServCargadas =
        this.inputServiciosCargados.value.length > 0
          ? JSON.parse(this.inputServiciosCargados.value)
          : [];
      ...
      this.servicios.forEach((s) => {
        ...
        const encontrado = listaCombinada.some(
          (serv) => parseInt(serv.id) === parseInt(servId),
        );
        if (encontrado) {
          s.checked = true;
        } else {
    ...
    if (this.inputServiciosCargados.value.length > 0) {
      ...
      if (listServCargadas && listServCargadas.length > 0) {
        this.servicios.forEach((s) => {
          const servId = s.value;
          const encontrado = listServCargadas.find(
            (serv) => serv.id_servicio === parseInt(servId),
          );
          if (encontrado) {
            s.disabled = true;
          }
  ...
  asociarEspecialidades(element) {
    ...
    if (valorEspSeleccionadas.length === 0) {
      ...
      if (element.target.checked) {
        ...
        const listaFiltrada = listEspDeserializadas.filter(
          (e) => e.id !== valueEspecialidad,
        );
  ...
  visualizarEspecialidades() {
    ...
    if (especialidadesSeleccionadas.length > 0) {
      ...
      const listaCargadas =
        especialidadesCargadas.length > 0
          ? JSON.parse(especialidadesCargadas)
          : [];
      this.containerEspecialidades.innerHTML = "";

      const listaCombinada = [
        ...listaCargadas,
        ...listaEspecialidades.filter(
          (e) => !listaCargadas.some((cargada) => cargada.id === e.id),
        ),
      ];
  ...
  eliminarEspecialidad(idEsp) {
    ...
    const listaEspecialidades =
      especialidadesSeleccionadas.length > 0
        ? JSON.parse(especialidadesSeleccionadas)
        : [];
    const listaCargadas =
      especialidadesCargadas.length > 0
        ? JSON.parse(especialidadesCargadas)
    ...
    if (
      ...
      listaEspecialidades.some((e) => e.id === idEsp)
    ) {
      const listaActualizada = listaEspecialidades.filter(
        (e) => e.id !== idEsp,
      );
      this.inputEspecialidades.value = JSON.stringify(listaActualizada);
      this.visualizarEspecialidades();
    } else if (
      listaCargadas.length > 0 &&
      listaCargadas.some((e) => e.id === idEsp)
    ) {
  ...
  asociarServicios(element) {
    ...
    if (valorServSeleccionadas.length === 0) {
      ...
      if (element.target.checked) {
        ...
        const listaFiltrada = listServDeserializadas.filter(
          (e) => e.id !== valueServicio,
        );
  ...
  visualizarServicios() {
    ...
    if (serviciosSeleccionados.length > 0) {
      ...
      const listaCargadas =
        serviciosCargados.length > 0 ? JSON.parse(serviciosCargados) : [];
  ...
  eliminarServicio(idServ) {
    ...
    const listaServicios =
      serviciosSeleccionados.length > 0
        ? JSON.parse(serviciosSeleccionados)
        : [];
    const listaCargadas =
      serviciosCargados.length > 0 ? JSON.parse(serviciosCargados) : [];
    if (
      listaServicios.length > 0 &&
      listaServicios.some((e) => e.id === idServ)
    ) {
      ...
      listaCargadas.length > 0 &&
      listaCargadas.some((e) => e.id === idServ)
    ) {
      const ruta = window.location.href;
      const idProfesional = ruta.split("id=")[1].split("&")[0];
  ...
  actualizarListaEspecialidades(especialidades) {
    ...
    if (especialidades.length === 0) {
      ulEspecialidades.innerHTML =
        '<li style="padding: 10px; text-align: center; color: #999;">Seleccione un servicio para ver las especialidades disponibles</li>';
  ...
  filtrarEspecialidadesNoDisponibles(especialidadesDisponibles) {
    ...
    const idsDisponibles = especialidadesDisponibles.map((e) =>
      e.id_especialidad.toString(),
    );
    const especialidadesFiltradas = especialidadesSeleccionadas.filter((esp) =>
      idsDisponibles.includes(esp.id.toString()),
    );
