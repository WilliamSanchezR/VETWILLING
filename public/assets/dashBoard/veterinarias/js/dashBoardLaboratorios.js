(function () {
  ...
  const tablaDetalleResultados = document.getElementById(
    "tablaDetalleResultados",
  );
  ...
  const resultadoPacienteInfo = document.getElementById(
    "resultadoPacienteInfo",
  );
  const resultadoPruebaTitulo = document.getElementById(
    "resultadoPruebaTitulo",
  );
  const resultadoOrdenPruebaId = document.getElementById(
    "resultadoOrdenPruebaId",
  );
  const estadoPruebaResultado = document.getElementById(
    "estadoPruebaResultado",
  );
  const observacionesResultado = document.getElementById(
    "observacionesResultado",
  );
  const selectPacienteLaboratorio = document.getElementById(
    "selectPacienteLaboratorio",
  );
  const selectPrioridadLaboratorio = document.getElementById(
    "selectPrioridadLaboratorio",
  );
  const motivoOrdenLaboratorio = document.getElementById(
    "motivoOrdenLaboratorio",
  );
  const observacionesOrdenLaboratorio = document.getElementById(
    "observacionesOrdenLaboratorio",
  );
  ...
  const modalResultado = new bootstrap.Modal(
    document.getElementById("modalResultadoLab"),
  );
  const modalNuevaOrden = new bootstrap.Modal(
    document.getElementById("modalNuevaOrdenLab"),
  );
  ...
  function badgeEstadoPrueba(estado) {
    ...
    if (estado === "Procesada" || estado === "Validada")
  ...
  function pintarStats(stats) {
    ...
    if (statCompletados)
      statCompletados.textContent = Number(stats.completados || 0);
    if (statPendientes)
      statPendientes.textContent = Number(stats.pendientes || 0);
    if (statCancelados)
      statCancelados.textContent = Number(stats.cancelados || 0);
  }
  ...
  function renderPacientesSelect() {
    selectPacienteLaboratorio.innerHTML =
  ...
  function renderCatalogoPruebas() {
    ...
    if (catalogoPruebas.length === 0) {
      ...
      listaPruebasCatalogo.innerHTML =
        '<div class="text-muted">No hay pruebas disponibles. Agrega registros al catálogo para crear órdenes.</div>';
      return;
    }
  ...
  function obtenerPruebasSeleccionadas() {
    return Array.from(
      document.querySelectorAll(".check-prueba-lab:checked"),
    ...
    apiGet("detalle", { id_orden_laboratorio: idOrden })
      ...
      .catch((error) => {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: error.message || "No se pudo cargar la orden.",
    ...
    apiGet("resultado", { id_orden_prueba: idOrdenPrueba })
      ...
      .catch((error) => {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: error.message || "No se pudo cargar el resultado.",
          confirmButtonColor: "#0a932c",
        });
      });
  }
  ...
  function guardarResultadoActual() {
    ...
    if (resultadoActual.tipo_resultado === "NUMERICO") {
      payload.resultado_numerico =
        document.getElementById("inputResultadoNumerico")?.value ?? "";
    } else if (resultadoActual.tipo_resultado === "BOOLEANO") {
      payload.resultado_booleano =
        document.getElementById("inputResultadoBooleano")?.value ?? "";
    } else {
      payload.resultado_texto =
        document.getElementById("inputResultadoTexto")?.value ?? "";
    }

    payload.unidad_resultado =
      document.getElementById("inputUnidadResultado")?.value ?? "";
    ...
    apiPost("guardar-resultado", payload)
      .then(() =>
        Promise.all([
          ...
          cargarStats(),
          apiGet("detalle", {
            id_orden_laboratorio: resultadoActual.id_orden_laboratorio,
          }),
        ]),
      )
      .then(([, , detalle]) => {
      ...
      .catch((error) => {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: error.message || "No se pudo guardar el resultado.",
          confirmButtonColor: "#0a932c",
        });
      });
  }
  ...
  function crearNuevaOrden() {
    const idPaciente = Number(selectPacienteLaboratorio.value);
    const pruebas = obtenerPruebasSeleccionadas();

    if (!idPaciente) {
      Swal.fire({
        icon: "warning",
        title: "Paciente requerido",
        text: "Selecciona un paciente antes de crear la orden.",
        confirmButtonColor: "#0a932c",
      });
      return;
    }
    if (!pruebas.length) {
      Swal.fire({
        icon: "warning",
        title: "Sin pruebas",
        text: "Selecciona al menos una prueba del catálogo.",
        confirmButtonColor: "#0a932c",
      });
      return;
    }
    ...
    apiPost("crear-orden", payload)
      ...
      .then(() => {
        ...
        Swal.fire({
          icon: "success",
          title: "Orden creada",
          text: "La orden de laboratorio fue registrada correctamente.",
          confirmButtonColor: "#0a932c",
          timer: 2000,
          showConfirmButton: false,
        });
  ...
  function exportarACSV() {
    ...
    try {
      ...
      let csv =
        "No.,Fecha,Propietario,Nombre Mascota,Animal,Raza,Laboratorios,Estado\n";
  ...
  $("#btnOrdenar").on("click", function () {
    ...
    Swal.fire({
      ...
      html:
        ...
        '<label style="display:block"><input type="radio" name="ord" value="1"> Fecha (más antigua primero)</label>' +
        '<label style="display:block"><input type="radio" name="ord" value="2" checked> Fecha (más reciente primero)</label>' +
        '<label style="display:block"><input type="radio" name="ord" value="3"> Propietario (A-Z)</label>' +
        '<label style="display:block"><input type="radio" name="ord" value="4"> Propietario (Z-A)</label>' +
        '<label style="display:block"><input type="radio" name="ord" value="5"> Mascota (A-Z)</label>' +
        "</div>",
      ...
      preConfirm: () => {
        const sel = document.querySelector('input[name="ord"]:checked');
  ...
  document.addEventListener("DOMContentLoaded", () => {
    Promise.all([cargarListado(), cargarStats(), cargarCatalogosModal()]).catch(
      (error) => {
        console.error(error);
      },
    );
