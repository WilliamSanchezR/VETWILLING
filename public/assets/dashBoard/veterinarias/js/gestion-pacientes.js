document.addEventListener("DOMContentLoaded", function () {
  ...
  const selectVersionHistorial = document.getElementById(
    "selectVersionHistorial",
  );
  const detallePacienteTitulo = document.getElementById(
    "detallePacienteTitulo",
  );
  ...
  const statVeterinariosActivos = document.getElementById(
    "statVeterinariosActivos",
  );
  ...
  const tratamientoMedicamento = document.getElementById(
  ...
  const tratamientoObservaciones = document.getElementById(
    "tratamientoObservaciones",
  );
  const btnGuardarTratamiento = document.getElementById(
  ...
  const consultaObservaciones = document.getElementById(
  ...
  const filtroEstadoTratamientos = document.getElementById(
    "filtroEstadoTratamientos",
  );
  const filtroFechaTratamientos = document.getElementById(
    "filtroFechaTratamientos",
  );
  ...
  const paginacionTratamientos = document.getElementById(
    "paginacionTratamientos",
  );
  ...
  const panelListadoAtenciones = document.getElementById(
    "panelListadoAtenciones",
  );
  const panelEdicionClinica = document.getElementById("panelEdicionClinica");
  const detalleContenidoHistorial = document.getElementById(
    "detalleContenidoHistorial",
  );
  ...
  function actualizarMiniStats(data) {
    ...
    listado.forEach(function (item) {
      ...
      const cruda =
        item && item.fecha_atencion
          ? String(item.fecha_atencion).replace(" ", "T")
          : "";
    ...
    statUltimaAtencion.textContent = fechaMaxima
      ? fechaMaxima.toLocaleDateString("es-CO", {
          day: "2-digit",
          month: "2-digit",
          year: "2-digit",
        })
  ...
  function actualizarResumenPaciente(registro) {
    ...
    if (!registro) {
      ...
      detallePacienteMeta.textContent =
        "Selecciona un registro en la tabla para iniciar la edición.";
      detalleEstadoEdicion.innerHTML =
        '<i class="bi bi-check-circle"></i> Listo para editar';
      return;
    }

    detallePacienteTitulo.textContent =
      registro.paciente_nombre || "Paciente sin nombre";
    detallePacienteMeta.textContent =
      (registro.especie || "Especie no registrada") +
      " · " +
      (registro.raza || "Raza no registrada") +
      " · Atención " +
      fechaHumana(registro.fecha_atencion);
    detalleEstadoEdicion.innerHTML =
      '<i class="bi bi-pencil-square"></i> Modo edición activo';
  }
  ...
  async function postJson(payload) {
    ...
    if (!response.ok || result.status !== "success") {
      throw new Error(
        result.message || "No fue posible completar la operación",
      );
    }
  ...
  function renderListadoModulo(contenedor, items, renderItem) {
    ...
    contenedor.innerHTML =
      '<div class="modulo-lista">' + items.map(renderItem).join("") + "</div>";
  }

  function obtenerBusqueda(tipo) {
    if (tipo === "vacuna")
      return (buscarVacunas.value || "").trim().toLowerCase();
    if (tipo === "tratamiento")
      return (buscarTratamientos.value || "").trim().toLowerCase();
    if (tipo === "consulta")
      return (buscarConsultas.value || "").trim().toLowerCase();
  ...
  function coincideFiltroAvanzado(tipo, item) {
    ...
    if (tipo === "tratamiento") {
      const estado = (filtroEstadoTratamientos.value || "")
        .trim()
        .toLowerCase();
      const fecha = (filtroFechaTratamientos.value || "").trim();
      const coincideEstado =
        !estado || String(item.estado || "").toLowerCase() === estado;
  ...
  function renderPaginacion(tipo, totalItems, totalPaginas) {
    ...
    for (let pagina = 1; pagina <= totalPaginas; pagina++) {
      html +=
        '<button type="button" class="btn-pagina ' +
  ...
  function editarModulo(tipo, idItem) {
    ...
    if (tipo === "vacuna") {
      ...
      btnGuardarVacuna.innerHTML =
        '<i class="bi bi-pencil-square"></i> Actualizar vacuna';
      return;
    }
    ...
    if (tipo === "tratamiento") {
      ...
      btnGuardarTratamiento.innerHTML =
        '<i class="bi bi-pencil-square"></i> Actualizar tratamiento';
      return;
    }
    ...
    if (tipo === "consulta") {
      ...
      btnGuardarConsulta.innerHTML =
        '<i class="bi bi-pencil-square"></i> Actualizar consulta';
      return;
    }
    ...
    if (tipo === "nota") {
      ...
      btnGuardarNota.innerHTML =
        '<i class="bi bi-pencil-square"></i> Actualizar nota';
    }
  }
  ...
  async function eliminarModulo(tipo, idItem) {
    ...
    if (tipo === "vacuna") {
      payload = {
        accion: "eliminar_vacuna",
        id_paciente: idPaciente,
        id_vacuna: Number(idItem),
      };
  ...
  function renderFichaClinica(ficha) {
    ...
    renderListadoModulo(
      listaVacunas,
      filtrarYPaginar("vacuna", ficha && ficha.vacunas ? ficha.vacunas : []),
      function (item) {
        return (
          '<div class="item-modulo">' +
          '<div class="item-modulo-head"><strong>' +
          escapeHtml(item.tipo_vacuna || "--") +
          "</strong><span>" +
          escapeHtml(fechaHumana(item.fecha_aplicacion)) +
          "</span></div>" +
          '<div class="item-modulo-body">Dosis: ' +
          escapeHtml(item.dosis || "--") +
          "</div>" +
          '<div class="item-modulo-foot">' +
          escapeHtml(item.observaciones || "Sin observaciones") +
          "</div>" +
          '<div class="item-modulo-actions">' +
          '<button type="button" class="btn-mini" data-modulo-action="editar" data-modulo="vacuna" data-id="' +
          escapeHtml(item.id_vacuna || "") +
          '" title="Editar"><i class="bi bi-pencil"></i></button>' +
          '<button type="button" class="btn-mini btn-mini-danger" data-modulo-action="eliminar" data-modulo="vacuna" data-id="' +
          escapeHtml(item.id_vacuna || "") +
    ...
    renderListadoModulo(
      ...
      filtrarYPaginar(
        "tratamiento",
        ficha && ficha.tratamientos ? ficha.tratamientos : [],
      ),
      function (item) {
        return (
          '<div class="item-modulo">' +
          '<div class="item-modulo-head"><strong>' +
          escapeHtml(item.medicamento || "--") +
          "</strong><span>" +
          ...
          escapeHtml(item.dosis || "--") +
          "</div>" +
          '<div class="item-modulo-foot">Inicio: ' +
          escapeHtml(fechaHumana(item.fecha_inicio)) +
          " · Fin: " +
          escapeHtml(
            fechaHumana(item.fecha_fin) === "--"
              ? "N/A"
              : fechaHumana(item.fecha_fin),
          ) +
          "</div>" +
          '<div class="item-modulo-actions">' +
          '<button type="button" class="btn-mini" data-modulo-action="editar" data-modulo="tratamiento" data-id="' +
          escapeHtml(item.id_tratamiento || "") +
          '" title="Editar"><i class="bi bi-pencil"></i></button>' +
          '<button type="button" class="btn-mini btn-mini-danger" data-modulo-action="eliminar" data-modulo="tratamiento" data-id="' +
          escapeHtml(item.id_tratamiento || "") +
          '" title="Eliminar"><i class="bi bi-trash"></i></button>' +
          "</div>" +
          "</div>"
    ...
    renderListadoModulo(
      ...
      filtrarYPaginar(
        "consulta",
        ficha && ficha.consultas ? ficha.consultas : [],
      ),
      function (item) {
        return (
          '<div class="item-modulo">' +
          ...
          "</strong><span>" +
          escapeHtml(fechaHoraHumana(item.fecha_consulta)) +
          "</span></div>" +
          '<div class="item-modulo-body">Diagnóstico: ' +
          escapeHtml(item.diagnostico || "Sin diagnóstico") +
          "</div>" +
          '<div class="item-modulo-foot">' +
          escapeHtml(item.observaciones || "Sin observaciones") +
          "</div>" +
          '<div class="item-modulo-actions">' +
          '<button type="button" class="btn-mini" data-modulo-action="editar" data-modulo="consulta" data-id="' +
          escapeHtml(item.id_consulta || "") +
          '" title="Editar"><i class="bi bi-pencil"></i></button>' +
          '<button type="button" class="btn-mini btn-mini-danger" data-modulo-action="eliminar" data-modulo="consulta" data-id="' +
          escapeHtml(item.id_consulta || "") +
          '" title="Eliminar"><i class="bi bi-trash"></i></button>' +
          "</div>" +
          "</div>"
    ...
    renderListadoModulo(
      listaNotas,
      filtrarYPaginar("nota", ficha && ficha.notas ? ficha.notas : []),
      function (item) {
        return (
          '<div class="item-modulo">' +
          '<div class="item-modulo-head"><strong>Nota clínica</strong><span>' +
          escapeHtml(fechaHoraHumana(item.created_at)) +
          "</span></div>" +
          '<div class="item-modulo-body">' +
          escapeHtml(item.nota || "--") +
          "</div>" +
          '<div class="item-modulo-actions">' +
          '<button type="button" class="btn-mini" data-modulo-action="editar" data-modulo="nota" data-id="' +
          escapeHtml(item.id_nota || "") +
          '" title="Editar"><i class="bi bi-pencil"></i></button>' +
          '<button type="button" class="btn-mini btn-mini-danger" data-modulo-action="eliminar" data-modulo="nota" data-id="' +
          escapeHtml(item.id_nota || "") +
          '" title="Eliminar"><i class="bi bi-trash"></i></button>' +
          "</div>" +
          "</div>"
  ...
  async function cargarFichaClinica(idPaciente) {
    ...
    try {
      ...
      limpiarModulos();
      showToast(
        error.message || "No se pudo cargar la ficha clínica.",
        "error",
      );
    }
  }
  ...
  async function cargarVersionesHistorial(idHistorial) {
    ...
    try {
      ...
      selectVersionHistorial.innerHTML =
        ...
        versiones
          .map(function (v) {
            ...
            const seleccionado =
              String(v.id_historial) === String(idHistorial) ? " selected" : "";
            return (
              '<option value="' +
              escapeHtml(v.id_historial) +
              '"' +
              seleccionado +
              ">" +
              version +
              ...
              fecha +
              " - " +
              escapeHtml(v.veterinario_responsable || "N/A") +
              "</option>"
            );
          })
          .join("");
      ...
      selectVersionHistorial.innerHTML =
        '<option value="">Error al cargar versiones</option>';
    }
  }
  ...
  function cargarDetalle(registro) {
    ...
    campos.fecha.value =
      toDateInput(registro.fecha_atencion) ||
    ...
    detalleVersion.textContent = registro.version_registro
      ? "v" + registro.version_registro
      : "Nueva";
    detalleActualizacion.textContent = fechaHoraHumana(
      registro.updated_at || registro.fecha_atencion,
    );
  ...
  function renderTabla() {
    if (!Array.isArray(registros) || registros.length === 0) {
      tablaBody.innerHTML =
        '<tr><td colspan="7" class="text-center py-3">No tienes pacientes vinculados o no hay historiales registrados.</td></tr>';
      limpiarFormulario();
      return;
    }
    ...
    tablaBody.innerHTML = registros
      .map(function (item) {
        const activo =
          pendingSelectId &&
          Number(item.id_historial || 0) === Number(pendingSelectId)
            ? "active"
            : "";
        const version = item.version_registro
          ? "v" + item.version_registro
          : "Nueva";
        ...
        return (
          ...
          '" ' +
          'data-id-historial="' +
          escapeHtml(item.id_historial || "") +
          '" ' +
          'data-id-paciente="' +
          escapeHtml(item.id_paciente || "") +
          '" ' +
          'data-paciente="' +
          escapeHtml(item.paciente_nombre || "") +
          '" ' +
          'data-especie="' +
          escapeHtml(item.especie || "") +
          '" ' +
          'data-raza="' +
          escapeHtml(item.raza || "") +
          '" ' +
          'data-fecha="' +
          escapeHtml(toDateInput(item.fecha_atencion)) +
          '" ' +
          'data-vet="' +
          escapeHtml(item.veterinario_responsable || "") +
          '" ' +
          'data-motivo="' +
          escapeHtml(item.motivo_consulta || "") +
          '" ' +
          'data-diagnostico="' +
          escapeHtml(item.diagnostico || "") +
          '" ' +
          'data-tratamiento="' +
          escapeHtml(item.tratamientos_aplicados || "") +
          '" ' +
          'data-medicacion="' +
          escapeHtml(item.medicacion_recetada || "") +
          '" ' +
          'data-observaciones="' +
          escapeHtml(item.observaciones_adicionales || "") +
          '" ' +
          'data-acceso="' +
          escapeHtml(acceso) +
          '" ' +
          'data-version="' +
          escapeHtml(item.version_registro || "") +
          '" ' +
          'data-updated-at="' +
          escapeHtml(item.updated_at || item.fecha_atencion || "") +
          '">' +
          '<td class="paciente-cell"><strong>' +
          escapeHtml(item.paciente_nombre || "--") +
          "</strong><small>" +
          escapeHtml((item.especie || "--") + " · " + (item.raza || "--")) +
          "</small></td>" +
          "<td>" +
          escapeHtml(fechaHumana(item.fecha_atencion)) +
          "</td>" +
          "<td>" +
          escapeHtml(item.veterinario_responsable || "--") +
          "</td>" +
          "<td>" +
          escapeHtml(item.motivo_consulta || "Sin registro") +
          "</td>" +
          '<td><span class="badge-acceso permitido"><i class="bi bi-check-circle"></i> ' +
          escapeHtml(acceso) +
          "</span></td>" +
    ...
    if (pendingSelectId) {
      filaInicial = tablaBody.querySelector(
        'tr[data-id-historial="' + pendingSelectId + '"]',
      );
    }
    if (!filaInicial) {
  ...
  async function cargarHistoriales(filtros) {
    tablaBody.innerHTML =
      '<tr><td colspan="7" class="text-center py-3">Cargando historiales...</td></tr>';
    ...
    try {
      ...
      tablaBody.innerHTML =
        ...
        escapeHtml(error.message || "Error de carga") +
        "</td></tr>";
      actualizarMiniStats([]);
      limpiarFormulario();
      showToast(
        error.message || "No se pudieron cargar los historiales.",
        "error",
      );
    }
  }
  ...
  btnExportarPdf.addEventListener("click", function () {
    ...
    const url =
      baseUrl +
      "/veterinaria/gestion-pacientes/pdf" +
  ...
  btnExportarFichaPdf.addEventListener("click", function () {
    ...
    if (idPaciente <= 0) {
      showToast(
        "Selecciona un paciente para exportar su ficha clínica.",
        "error",
      );
      return;
    }
    ...
    const url =
      baseUrl +
      "/veterinaria/gestion-pacientes/pdf-mascota?" +
      params.toString();
    window.open(url, "_blank");
  });
  ...
  btnGuardar.addEventListener("click", async function () {
    ...
    if (!payload.fecha_atencion || !payload.motivo_consulta) {
      showToast(
        "La fecha de atención y el motivo de consulta son obligatorios.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        error.message || "Error al guardar el historial clínico.",
        "error",
      );
    }
  });
  ...
  btnGuardarTratamiento.addEventListener("click", async function () {
    ...
    if (idPaciente <= 0) {
      showToast(
        "Selecciona un paciente antes de registrar tratamientos.",
        "error",
      );
      return;
    }
    ...
    if (!payload.medicamento || !payload.dosis || !payload.fecha_inicio) {
      showToast(
        "Medicamento, dosis y fecha de inicio son obligatorios.",
        "error",
      );
      return;
    }
  ...
  btnGuardarConsulta.addEventListener("click", async function () {
    ...
    if (idPaciente <= 0) {
      showToast(
        "Selecciona un paciente antes de registrar consultas.",
        "error",
      );
      return;
    }
    ...
    const payload = {
      accion:
        edicionModulo.consulta > 0 ? "actualizar_consulta" : "guardar_consulta",
      id_paciente: idPaciente,
  ...
  btnGuardarNota.addEventListener("click", async function () {
    ...
    try {
      ...
      showToast(
        error.message || "No se pudo guardar la nota clínica.",
        "error",
      );
    }
  });

  [listaVacunas, listaTratamientos, listaConsultas, listaNotas].forEach(
    function (contenedor) {
  ...
  [buscarVacunas, buscarTratamientos, buscarConsultas, buscarNotas].forEach(
    function (input, idx) {
  ...
  selectVersionHistorial.addEventListener("change", async function () {
    ...
    if (registroSeleccionado) {
      const filaTabla = tablaBody.querySelector(
        'tr[data-id-historial="' + idHistorialSeleccionado + '"]',
      );
      if (filaTabla) {
        filaTabla.click();
      } else {
