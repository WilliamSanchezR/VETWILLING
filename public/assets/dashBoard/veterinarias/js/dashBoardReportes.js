      ...
      datasets: [
        {
          ...
          backgroundColor: labels.map(
            (_, idx) => coloresServicios[idx % coloresServicios.length],
          ),
          borderWidth: 0,
        },
      ],
...
function renderLeyendaServicios(servicios) {
  ...
  if (!servicios.length) {
    contenedor.innerHTML =
      '<div class="text-muted small p-3">Sin datos para el periodo seleccionado.</div>';
    return;
  }
...
function renderResumen(resumen, meta) {
  ...
  setText(
    ...
    Number(resumen.citas_atendidas || 0).toLocaleString("es-CO"),
  );
  setText(
    "reporteNuevosPacientes",
    Number(resumen.nuevos_pacientes || 0).toLocaleString("es-CO"),
  );
  setText(
    "reporteCumplimiento",
    `${Number(resumen.cumplimiento || 0).toFixed(1)}%`,
  );
  setText(
    "reporteTotalCitas",
    `${Number(resumen.total_citas || 0).toLocaleString("es-CO")} citas`,
  );
...
function renderTratamientos(tratamientos) {
  ...
  const maximo = tratamientos.length
    ? Math.max(...tratamientos.map((item) => Number(item.total || 0)))
    : 0;

  if (!tratamientos.length) {
    contenedor.innerHTML =
      '<div class="text-muted small p-3">Sin tratamientos registrados para el periodo seleccionado.</div>';
    return;
  }

  contenedor.innerHTML = tratamientos
    .map((item) => {
...
function renderEspecies(especies) {
  ...
  if (!especies.length) {
    contenedor.innerHTML =
      '<div class="text-muted small p-3">Sin especies registradas para el periodo seleccionado.</div>';
    return;
  }
...
function renderFinanciero(financiero) {
  ...
  if (!filas.length) {
    tbody.innerHTML =
      '<tr><td colspan="7">Sin información financiera para el año seleccionado.</td></tr>';
    return;
  }
  ...
  for (let i = 0; i < 5; i++) {
    celdasTotales.push(
      `<td><strong>${formatoMoneda(totales[i] || 0)}</strong></td>`,
    );
  }
...
function renderAsignacionesActivas(asignaciones) {
  ...
  if (!asignaciones || !asignaciones.length) {
    contenedor.innerHTML =
      '<div class="text-muted small p-3">No hay pacientes asignados activos.</div>';
    return;
  }

  contenedor.innerHTML = asignaciones
    .map((item) => {
      const ultimaVisita = item.ultima_visita
        ? new Date(item.ultima_visita).toLocaleDateString("es-CO")
        : "Sin visitas";
...
function renderHistorialAsignaciones(historial) {
  const contenedor = document.getElementById(
    "listaHistorialAsignacionesReporte",
  );
  if (!contenedor) return;

  if (!historial || !historial.length) {
    contenedor.innerHTML =
      '<div class="text-muted small p-3">Sin movimientos de asignación en el periodo.</div>';
    return;
  }
...
function renderResumenEstados(resumenEstados) {
  ...
  setText(
    ...
    Number(resumenEstados.canceladas || 0).toLocaleString("es-CO"),
  );
  setText(
    "reporteCitasPendientes",
    Number(resumenEstados.pendientes || 0).toLocaleString("es-CO"),
  );
}
...
function renderDetalleCitas(detalle) {
  ...
  if (!detalle || !detalle.length) {
    tbody.innerHTML =
      '<tr><td colspan="8" class="text-muted text-center p-3">Sin citas para el periodo y filtros seleccionados.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = detalle
    .map((item) => {
      ...
      const fecha = item.fecha
        ? new Date(item.fecha.replace(" ", "T")).toLocaleDateString("es-CO", {
            day: "2-digit",
            month: "short",
            year: "numeric",
            hour: "2-digit",
...
function abrirModalEnviarFicha(idPaciente = 0, nombrePaciente = "") {
  ...
  if (inputNombre)
    inputNombre.value = nombrePaciente || "Seleccionar desde la tabla";
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("modalEnviarFicha"),
  );
  modal.show();
}
...
async function enviarFichaClinica(idPaciente, mensaje) {
  ...
  if (!idPaciente || idPaciente <= 0) {
    if (alerta) {
      ...
      alerta.textContent = "Selecciona un paciente desde la tabla de citas.";
    }
    return;
  }
  ...
  try {
    ...
    if (data.status === "success") {
      if (alerta) {
        ...
        alerta.textContent = data.message || "Correo enviado correctamente.";
      }
    } else {
      if (alerta) {
        alerta.className = "alert alert-danger";
        alerta.textContent = data.message || "Error al enviar el correo.";
      }
    }
  } catch (err) {
    if (alerta) {
      alerta.className = "alert alert-danger";
      alerta.textContent = "Error de red al intentar enviar.";
    }
    console.error(err);
  } finally {
    if (btn) {
...
async function cargarDashboardReportes() {
  try {
    ...
    crearGraficoIngresos(
      payload.ingresos_mensuales?.labels || [],
      payload.ingresos_mensuales?.data || [],
    );
...
function configurarEventos() {
  document.querySelectorAll(".boton-periodo").forEach((button) => {
    button.addEventListener("click", function () {
      document
        .querySelectorAll(".boton-periodo")
      ...
      if (rangoDiv) {
        rangoDiv.style.display =
          periodoActual === "personalizado" ? "flex" : "none";
      }
  ...
  if (btnConfirmar) {
    btnConfirmar.addEventListener("click", () => {
      const idPaciente = parseInt(
        document.getElementById("enviarFichaIdPaciente")?.value || "0",
        10,
      );
      const mensaje =
        document.getElementById("enviarFichaMensaje")?.value || "";
  ...
  // "Ver Detalle Completo" — hace scroll a la tabla financiera
  const btnDetalle = document.getElementById("btnVerDetalleFinanciero");
  if (btnDetalle) {
    btnDetalle.addEventListener("click", function () {
      const tablaFinanciera = document.querySelector(
        ".tarjeta-tabla-financiera-mejorada .contenedor-tabla-scroll",
      );
      if (tablaFinanciera) {
        tablaFinanciera.scrollIntoView({ behavior: "smooth", block: "center" });
        tablaFinanciera.style.outline = "2px solid #0a932c";
        setTimeout(function () {
          tablaFinanciera.style.outline = "";
        }, 2000);
      }
    });
  }
...
function renderInventarioVet(inv, productos) {
  ...
  if (!productos.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="text-center text-muted py-3"><i class="bi bi-check-circle text-success me-1"></i>Sin insumos próximos a vencer</td></tr>';
    return;
  }
  const esc = (s) => {
